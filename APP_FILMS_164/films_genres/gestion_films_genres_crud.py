"""
    ...
    Gestions des "routes" FLASK et des données pour l'association entre les films et les Pays.
"""
...
"""
    ...
    But : Afficher les films avec les Pays associés pour chaque film.
    
    Paramètres : id_pays_sel = 0 >> tous les films.
                 id_pays_sel = "n" affiche le film dont l'id est "n"
        ...
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_film, nom_film, duree_film, description_film, cover_link_film, date_sortie_film,
                                                            GROUP_CONCAT(nom) as GenresFilms FROM t_genre_film
                                                            RIGHT JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                                            LEFT JOIN t_pays ON t_genre.id_pays = t_genre_film.fk_genre
                                                            GROUP BY id_film"""
                if id_film_sel == 0:
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    ...
                    flash(f"Données films et Pays affichés !!", "success")
...
"""
    ...
    Récupère la liste de tous les Pays du film sélectionné par le bouton "MODIFIER" de "films_genres_afficher.html"
    
    Dans une liste déroulante particulière (tags-selector-tagselect), on voit :
    1) Tous les Pays contenus dans la "t_genre".
    2) Les Pays attribués au film selectionné.
    3) Les Pays non-attribués au film sélectionné.
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_pays_afficher = """SELECT id_pays, nom FROM t_pays ORDER BY id_pays ASC"""
            ...
            # 2) Sélection des Pays "déjà" attribués pour le film.
            # 3) Sélection des Pays "pas encore" attribués pour le film choisi.
            ...
            # les Pays qui ne sont pas encore sélectionnés.
            lst_data_genres_films_non_attribues = [item['id_pays'] for item in data_genres_films_non_attribues]
            ...
            # Dans le composant "tags-selector-tagselect" on doit connaître
            # les Pays qui sont déjà sélectionnés.
            lst_data_genres_films_old_attribues = [item['id_pays'] for item in data_genres_films_attribues]
            ...
            # Extrait les valeurs contenues dans la table "t_genres", colonne "nom"
            # Le composant javascript "tagify" pour afficher les tags n'a pas besoin de l'id_pays
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            # Récupère la liste des Pays qui ne sont pas associés au film sélectionné.
            ...
            # Récupère la liste des Pays qui sont associés au film sélectionné.
            ...
            # Récupère ce que l'utilisateur veut modifier comme Pays dans le composant "tags-selector-tagselect"
            ...
            # OM 2021.05.02 Une liste de "id_pays" qui doivent être effacés de la table intermédiaire "t_genre_film".
            ...
            # Une liste de "id_pays" qui doivent être ajoutés à la "t_genre_film"
            ...
            # "fk_film"/"id_film" et "fk_genre"/"id_pays" dans la "t_genre_film"
            strsql_insert_pays_film = """INSERT INTO t_genre_film (id_pays_film, fk_genre, fk_film)
                                                    VALUES (NULL, %(value_fk_genre)s, %(value_fk_film)s)"""

            # SQL pour effacer une (des) association(s) existantes entre "id_film" et "id_pays" dans la "t_genre_film"
            strsql_delete_genre_film = """DELETE FROM t_genre_film WHERE fk_genre = %(value_fk_genre)s AND fk_film = %(value_fk_film)s"""

            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des Pays à INSÉRER dans la "t_genre_film".
                # Si la liste est vide, la boucle n'est pas parcourue.
                for id_pays_ins in lst_diff_genres_insert_a:
                    # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
                    # et "id_pays_ins" (l'id du genre dans la liste) associé à une variable.
                    valeurs_film_sel_genre_sel_dictionnaire = {"value_fk_film": id_film_selected,
                                                               "value_fk_genre": id_pays_ins}

                    mconn_bd.execute(strsql_insert_pays_film, valeurs_film_sel_genre_sel_dictionnaire)

                # Pour le film sélectionné, parcourir la liste des Pays à EFFACER dans la "t_genre_film".
                # Si la liste est vide, la boucle n'est pas parcourue.
                for id_pays_del in lst_diff_genres_delete_b:
                    # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
                    # et "id_pays_del" (l'id du genre dans la liste) associé à une variable.
                    valeurs_film_sel_genre_sel_dictionnaire = {"value_fk_film": id_film_selected,
                                                               "value_fk_genre": id_pays_del}
...
"""
    ...
    Récupère la liste de tous les Pays du film sélectionné par le bouton "MODIFIER" de "films_genres_afficher.html"
    Nécessaire pour afficher tous les "TAGS" des Pays, ainsi l'utilisateur voit les Pays à disposition

    On signale les erreurs importantes
"""


def Pays_films_afficher_data(valeur_id_film_selected_dict):
    print("valeur_id_film_selected_dict...", valeur_id_film_selected_dict)
    try:

        strsql_film_selected = """SELECT id_film, nom_film, duree_film, description_film, cover_link_film, date_sortie_film, GROUP_CONCAT(id_pays) as GenresFilms FROM t_genre_film
                                        INNER JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                        INNER JOIN t_pays ON t_genre.id_pays = t_genre_film.fk_genre
                                        WHERE id_film = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT id_pays, nom FROM t_pays WHERE id_pays not in(SELECT id_pays as idGenresFilms FROM t_genre_film
                                                    INNER JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                                    INNER JOIN t_pays ON t_genre.id_pays = t_genre_film.fk_genre
                                                    WHERE id_film = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_film, id_pays, nom FROM t_genre_film
                                            INNER JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                            INNER JOIN t_pays ON t_genre.id_pays = t_genre_film.fk_genre
                                            WHERE id_film = %(value_id_film_selected)s"""
